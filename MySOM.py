...
class MiniSom(object):
    ...
    # kmeans clustering
    def weightsclustering(self, data, k):
        from sklearn.cluster import KMeans
        # reshape self._weights
        ...
        for i1 in range(2, k + 2):
            ...
            cl_var = (1.0 / (N - m) / d) * sum([sum(distance.cdist(weights[np.where(labels == i)], [centers[0][i]],
            ...
            gybicsc[i1 - 2] = BIC
        index = np.argwhere(gybicsc == max(gybicsc))
        if len(index) > 1:
            index = min(index)
        index = index + 2
        index = int(index)
        ...
        # reverse shape to make a label map
        labelmap = np.zeros((r, c))
        n0 = -1
        for i1 in range(r):
            for j1 in range(c):
                n0 = n0 + 1
                labelmap[i1, j1] = labels1[n0]
        q = np.zeros(data.shape)
        for i, x in enumerate(data):
            q[i] = labelmap[self.winner(x)]
...
def MySOM(Im, ImageType, k):
    ...
    som = MiniSom(somx, somy, 3, sigma=(somx / 3), learning_rate=0.3)  # 3x3 = 9 final colors
    som.random_weights_init(pixels)
    starting_weights = som.get_weights().copy()  # saving the starting weights
    som.train_batch(pixels, 100)

    qnt = som.weightsclustering(pixels, k)
    clustered = np.zeros(Im.shape)
    for i, q in enumerate(qnt):  # place the quantized values into a new image
        clustered[np.unravel_index(i, dims=(Im.shape[0], Im.shape[1]))] = q

    ClusterIm = clustered[:, :, 0]
    return ClusterIm
