...
class MiniSom(object):
    ...
    # kmeans clustering
    def weightsclustering(self, data):
...
def MySOM10(Im, ImageType, k):
    if ImageType == 'Hyper':
        ...
        pixels = pca.fit_transform(Im)   
    if ImageType == 'RGB':

        r, c = Im.shape[0:2]
        pixels = np.zeros(( r * c,3))
        n = -1
        for i in range(r):
            for j in range(c):
                n = n + 1
    ...
    # som size
        ...
        labels = som.weightsclustering(pixels)
        metric.append(metrics.calinski_harabaz_score(pixels, labels))
    ...
    index = index + 2
    index = int(index)
    som = MiniSom(1, index, 3, sigma=(somx / 3), learning_rate=0.3)
    som.random_weights_init(pixels)
